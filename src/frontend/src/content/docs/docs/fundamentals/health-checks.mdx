...
import healthChecksDashboardStatus from '@assets/docs/fundamentals/health-checks-dashboard-status.png';
...
The `AddServiceDefaults` and `MapDefaultEndpoints` methods also apply various configurations to your app beyond just health checks, such as OpenTelemetry and [service discovery](/docs/fundamentals/service-discovery/) configurations.
...
<Image src={healthChecksDashboardStatus} alt="Screenshot of the Aspire dashboard showing health check status for resources" />
