...
[MySQL](https://www.mysql.com/) is an open-source Relational Database Management System (RDBMS) that uses Structured Query Language (SQL) to manage and manipulate data. The MySQL integration enables you to connect to existing MySQL databases or create new instances from Aspire with the `mysql` container image.
...
<Aside type="tip">
If you'd rather connect to an existing MySQL server, call `AddConnectionString` instead. For more information, see [Reference existing resources](/docs/get-started/resources/).
</Aside>
