...
The SQL Server hosting integration models the server as the `SqlServerServerResource` type and the database as the `SqlServerDatabaseResource` type. To access these types and APIs, add the [📦 Aspire.Hosting.SqlServer](https://www.nuget.org/packages/Aspire.Hosting.SqlServer) NuGet package in the [AppHost](/docs/get-started/app-host/) project.
...
By default, Aspire integrations enable [health checks](/docs/fundamentals/health-checks/) for all services. For more information, see [Aspire integrations overview](/docs/integrations/overview/).
