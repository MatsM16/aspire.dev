...
<SimpleAppHostCode collapse={['3-17']} />
...
<SimpleAppHostCode mark={{ range: '4-6' }} collapse={['8-19']} />
...
<SimpleAppHostCode mark={{ range: '9-11' }} collapse={['1-6', '13-19']} />
...
<Aside type="note">
This example uses a Node.js (React) frontend, but Aspire treats frontends as executable services—any language or framework works.
</Aside>

<SimpleAppHostCode mark={{ range: '14-17' }} collapse={['1-11']} />

Key points:

- `.AddViteApp("frontend", "../frontend")` registers the (Vite-based) frontend project as a service named `frontend`.
...
<Aside type="tip">
`.WithNpmPackageInstallation()` is supplied by the Community Toolkit package: `📦 CommunityToolkit.Aspire.Hosting.NodeJS.Extensions`.
</Aside>
...
<LearnMore>
  Dive deeper into Aspire's orchestration and the [Resource Model](/docs/architecture/resource-model/).
</LearnMore>
...
<LearnMore>
  For finer-grained lifecycle control, see the [well-known lifecycle events](/docs/architecture/resource-model/#well-known-lifecycle-events).
</LearnMore>
