...
- **Orchestration** — Simplify service startup, health checks, and service discovery
- **Observability** — Get enhanced telemetry, logging, and distributed tracing through the Aspire dashboard
- **Development experience** — Improve local development with automatic service coordination
- **Gradual adoption** — Add Aspire features incrementally without disrupting your existing architecture
...
1. **Initialize Aspire support** — Use `aspire init` to add the AppHost project
2. **Add ServiceDefaults (optional)** — Integrate common observability and resilience patterns
3. **Add integrations (optional)** — Replace existing client libraries with Aspire integrations
4. **Run and verify** — Test your application with Aspire orchestration
...
After running `aspire init`, your solution will have a file-based AppHost:
...
```csharp title="apphost.cs — Example file-based orchestration"
...
When you add a hosting integration (like Redis) to your AppHost and reference it from a dependent resource, Aspire automatically injects the necessary configuration into that resource. This includes connection strings, URLs, environment variables, and other service-specific settings. Your services receive this configuration through the standard .NET configuration system, eliminating the need for manual connection string management.
...
3. Update your AppHost to reference the integration:
   ...
   ```csharp title="AppHost.cs — Add Redis integration"
   var builder = DistributedApplication.CreateBuilder(args);
...
- **Add more integrations** — Explore [Aspire integrations](/integrations/) to simplify working with databases, caches, and message queues
- **Configure ServiceDefaults** — Learn about [Service Defaults](/fundamentals/service-defaults/) to add standardized observability
- **Deploy your app** — Follow the [Deploy your first app](/get-started/deploy-first-app/) tutorial to deploy your Aspire-enabled application
